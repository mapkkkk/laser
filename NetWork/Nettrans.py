...
5。14。2
这个文件实现了Application的远程协议,考虑到实际使用的并不频繁,所以还是先放在这里
"""
import socket
import sys
import time
...
class func_basetrans(object):
    def __init__(self, fc: class_application):
        self.fc = fc
...
class FC_Server(class_application):
    """
    飞控服务器, 负责转发指令到飞控
    """
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self._manager = None
        self._proxy = func_basetrans(self)
    ...
    def init(self, port=5654, authkey=b"fc"):
        ...
        if not self.running:
            raise Exception(
                "Serial listening must be started before manager_init")
    ...
    def run(self):
        ...
        logger.info(
            f"[FC_Server] Manager serving on {address}, {local_address}")
        self._manager.get_server().serve_forever()


class FC_Client(class_application):
    """
    飞控客户端, 与服务器通信
    """
        ...
        while self.running:
            ...
            try:
                ...
                if "WinError" in str(e) or "Broken pipe" in str(e):
                    logger.warning(
                        "[FC_Client] Connection lost, trying to reconnect")
                    self.running = False
                    for i in range(3):
                        logger.warning(
                            f"[FC_Client] Trying to reconnect {i+1}/3")
                        try:
                            self._manager = None  # 先析构
                            self.connect(*self._last_connection_args)
                        except:
                            logger.warning(
                                f"[FC_Client] Reconnect failed {i+1}/3")
                            time.sleep(1)
                        else:
                        ...
                        logger.error(
                            "[FC_Client] All reconnect failed, closing")
